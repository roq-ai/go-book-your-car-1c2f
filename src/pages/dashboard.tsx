...
import { getMechanics } from 'apiSdk/mechanics';
...
function DashboardPage() {
  ...
  const mechanicStat = useSWR<PaginatedInterface<MechanicInterface>>(
    () => `/mechanics`,
    () => getMechanics({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Mechanics',
      result: mechanicStat?.data?.totalCount,
      isLoading: mechanicStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
